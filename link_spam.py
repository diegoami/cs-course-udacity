...
def is_there_path(graph,page,node,k):
    if page == node:
        return True
    if k <= 0:
        return False
    if page in graph:
        pages_of_page = graph[page]
        for page_of_page in pages_of_page:
            if is_there_path(graph, page_of_page, node, k-1):
                return True
        return False    
    else:
        return False
        ...
        for page in graph:
            ...
            for node in graph:
                if page in graph[node]:
                    if not is_there_path(graph,page,node,k):
